...
from nanda_governance.nanda import approval_to_integrity_facts
...
class TestCreateProvenanceWithApprovalImportError:
    ...
    when nanda-model-integrity-layer is not installed."""

    def test_raises_import_error_when_integrity_not_available(self) -> None:
        from nanda_governance.nanda import create_provenance_with_approval
        ...
        with (
            patch("nanda_governance.nanda.has_nanda_integrity", return_value=False),
            pytest.raises(ImportError, match="nanda-model-integrity-layer"),
        ):
